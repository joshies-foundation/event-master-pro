...
export function gameboardSpaceFormFactory(
  ...
  const gameboardSpaceFormGroup: ModelFormGroup<GameboardSpaceTypeForm> =
    formBuilder.nonNullable.group({
      ...
      alternativeActivity: [''],
      duelGames: [''],
      specialEvents: createFormArray<SpecialSpaceEvent>(formBuilder, [
        {
          name: '',
          effect: {
            type: SpecialSpaceEventType.PlayerGainsPointsBasedOnGameScore,
            data: {
              pointsLabelSingular: '',
              pointsLabelPlural: '',
  ...
  const gameboardSpaceForm = {
    ...
    fields: computed((): FormField[] => [
      ...
      ...gameboardSpaceFormGroup.controls.specialEvents.controls
        .map((control, index): FormField[] => [
          {
            type: FormFieldType.Text,
            name: `special-event-${index + 1}-name`,
            label: `Special Event Option ${index + 1}`,
            ...
            control:
              gameboardSpaceFormGroup.controls.specialEvents.at(index).controls
                .name,
          },
          {
            type: FormFieldType.Dropdown,
            name: `special-event-${index + 1}-effect`,
            label: `Event Type`,
            required: gameboardSpaceEffect() === GameboardSpaceEffect.Special,
            visible: gameboardSpaceEffect() === GameboardSpaceEffect.Special,
            placeholder: 'Event Type',
            options: Object.values(SpecialSpaceEventType).map((eventType) => ({
              label: eventType
                .replace(/_/g, ' ')
                .replace(/\b\w/g, (char) => char.toUpperCase()),
              value: eventType,
            ...
            control:
              gameboardSpaceFormGroup.controls.specialEvents.at(index).controls
                .effect.controls.type,
          },
          {
            type: FormFieldType.Text,
            name: `special-event-${index + 1}-pointsLabelSingular`,
            label: `Points Label Singular`,
            required:
              gameboardSpaceEffect() === GameboardSpaceEffect.Special &&
              gameboardSpaceFormGroup.controls.specialEvents
                .at(index)
                .controls.effect.controls.type.getRawValue() ===
            ...
            control: (
              gameboardSpaceFormGroup.controls.specialEvents.at(index).controls
                .effect.controls.data
                .controls as ModelFormGroupType<PlayerGainsPointsBasedOnGameScoreSpecialSpaceEffectData>
            ).pointsLabelSingular,
          },
          {
            type: FormFieldType.Text,
            name: `special-event-${index + 1}-pointsLabelPlural`,
          ...
          },
          {
            type: FormFieldType.Number,
            showButtons: true,
            name: `special-event-${index + 1}-sessionPointsPerGamePoint`,
            label: `Session Points per Game Point`,
            required:
              gameboardSpaceEffect() === GameboardSpaceEffect.Special &&
              gameboardSpaceFormGroup.controls.specialEvents
                .at(index)
